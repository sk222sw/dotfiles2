...
-- Component extraction keymap with TypeScript
vim.keymap.set("v", "<leader>rc", function()
  ...
  if component_name ~= "" then
    ...
    local new_component = string.format("interface %sProps {\n  // Add props here\n}\n\nconst %s: React.FC<%sProps> = () => {\n  return (\n    %s\n  );\n};\n\nexport default %s;", 
      component_name, component_name, component_name, component_content, component_name)
    
    -- Replace selection with component usage
    vim.api.nvim_buf_set_lines(0, start_pos[2] - 1, end_pos[2], false, {"<" .. component_name .. " />"})
    
    -- Create new file
    local filename = string.format("%s.tsx", component_name)
